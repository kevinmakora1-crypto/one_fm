...
class TestRecruitmentDashboard(FrappeTestCase):
	...
	def test_get_dashboard_data_aggregation(self, mock_get_all, mock_get_roles):
		...
		# PMR: Total 52 (Male: 51, Female: 1)
		mock_pmr = [
			frappe._dict({"name": "PMR-001", "erf": "ERF-2026-00043", "gender": "Male", "count": 51}),
			frappe._dict({"name": "PMR-002", "erf": "ERF-2026-00043", "gender": "Female", "count": 1})
		]

		# PMR Linked Candidates
		mock_linked = []

		# CCP: status Joined (but not linked to PMR -> Joined Without PR)
		mock_ccp = [
		...
		mock_offers = [
			frappe._dict({"name": "JO-001", "one_fm_erf": "ERF-2026-00043", "status": "Accepted", "job_applicant": "JA-001"}),
			frappe._dict({"name": "JO-002", "one_fm_erf": "ERF-2026-00043", "status": "Accepted", "job_applicant": "JA-002"}),
			frappe._dict({"name": "JO-003", "one_fm_erf": "ERF-2026-00043", "status": "Accepted", "job_applicant": "JA-003"}),
			# An Awaiting Response offer (Overseas)
			frappe._dict({"name": "JO-004", "one_fm_erf": "ERF-2026-00043", "status": "Awaiting Response", "job_applicant": "JA-004"})
		]
